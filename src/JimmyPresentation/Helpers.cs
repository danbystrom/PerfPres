...
namespace JimmyPresentation
{
    public unsafe static class Helpers
    {
        ...
        public static int CopyToByteArray(this LargeValueObjectAsStruct[] array, ref byte[] buffer)
        {
            var size = array.Length * Marshal.SizeOf(typeof(LargeValueObjectAsStruct));
            if (buffer == null || buffer.Length < size)
                buffer = new byte[size];
        ...
        public static void CopyFromByteArray(this LargeValueObjectAsStruct[] array, byte[] buffer, int? elements = null)
        {
            var size = elements.GetValueOrDefault(array.Length) * Marshal.SizeOf(typeof(LargeValueObjectAsStruct));
            if (size > buffer.Length)
                throw new IndexOutOfRangeException();
